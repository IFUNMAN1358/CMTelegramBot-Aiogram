...
from src.application.fsm_states.auth.FSMAuth import FSMLogin
...
async def callback_query_fsm_login(call: CallbackQuery, state: FSMContext):
...
async def receiving_password(message: Message, state: FSMContext):
    ...
    # Сохраняем данные из состояний в словаре и очищаем состояние
    await state.update_data(password=message.text.strip())
    auth_dct = await state.get_data()
    await state.clear()

    # Удаляем все сообщения с чувствительной информацией
    ...
    # Пост запрос в CM на получение данных для сессии
    ...
    if result["success"]:
        ...
        # Делаем объект сессии и сохраняем в редиску
